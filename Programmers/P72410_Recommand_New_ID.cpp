...
string solution(string new_id) {
    ...
    // step 3. cannot be used continuously period(.)  
    int period_cnt = 0 ;
    for(auto s : temp){
        if(s == '.')
            period_cnt += 1 ;
    ...
    // step 4. remove period(.) at front and end
    if(temp.front() != '.')
        answer += temp.front() ;
    ...
    // step 5. if new_id is empty string, new_id = "a"
    if(temp.empty() || temp.size() == 0)
        answer = "a" ;
    else
        answer = temp ;
    
    // step 6. if string size over than 16, erase over-word
    if(15 < answer.size()){
        if(answer.at(14) == '.')
    ...
    // step 7. if string length less than 2 word
    while(answer.size() < 3)
        answer += answer.back() ;
    return answer;
}