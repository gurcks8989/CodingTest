/*
신고 결과 받기
문제 출처: https://programmers.co.kr/learn/courses/30/lessons/92334?language=cpp
...
vector<int> solution(vector<string> id_list, vector<string> report, int k) {
    vector<vector<string>> reportResult(id_list.size());
    vector<int> answer(id_list.size());
    sort(report.begin(), report.end());
    report.erase(unique(report.begin(), report.end() ), report.end());
    for(string declare : report){
        stringstream ss(declare) ;
        string reporter, reported ;
        ss >> reporter >> reported ;
        auto it = find(id_list.begin(), id_list.end(), reported);
        reportResult[it - id_list.begin()].push_back(reporter) ;
    }
    for(auto users : reportResult){
        if(k <= users.size()){
            for(auto user : users){
                auto it = find(id_list.begin(), id_list.end(), user);
                answer[it - id_list.begin()] += 1 ;
            }
        }
    }
