...
int main(){
    ...
    for(int i = 0 ; i < T ; i++){
        ...
        cin.ignore() ;
        getline(cin, line) ;
        arr = line.substr(1, line.size()-2) ;
        if(0 < arr.size()){
            int index = 1, length = 0 ;
            for(int j = 1 ; j < line.size()-1 ; j++){
                if(line[j] == ','){
                    length = j - index ;
                    nums.push_back(stoi(line.substr(index, length)));
                    index = j + 1 ;
                }
            }
            nums.push_back(stoi(line.substr(index, line.size()-index)));
        }
