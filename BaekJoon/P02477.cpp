...
반시계방향으로 남쪽으로 30m, 동쪽으로 60m, 남쪽으로 20m, 
...
int main(){
    ...
    cin.tie(NULL) ;
    cout.tie(NULL) ;
    int K, vertical = 0, horizon = 0 ;
    cin >> K ;
    vector<pair<int, int>> inputs(6) ;
    for(int i = 0 ; i < 6 ; i++){
        int type, meter ;
        cin >> type >> meter ;
        inputs[i] = make_pair(type, meter) ;
        if(3 <= type)
            vertical += meter ;
        else
            horizon += meter ;
    ...
    int index = -1 ;
    for(int i = 0 ; i < 6 ; i++){
        if(inputs[i].second == horizon || inputs[i].second == vertical){
            if(i == 5)
                i = 0 ;
            else
                i++ ;
    ...
    cout << (vertical * horizon - inputs[x].second * inputs[y].second) * K << "\n" ;
    return 0 ;
}