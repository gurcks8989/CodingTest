...
bool isInsideCircle(pair<int, int> & a, pair<int, int> & b, int r){
    return squared(a.first-b.first) + squared(a.second-b.second) < squared(r) ;
}
...
int main(){
    ...
    for(int i = 0 ; i < T ; i++){
        int n, answer = 0 ;
        pair <int, int> startPoint, arrivalPoint ;
        cin >> startPoint.first >> startPoint.second >> arrivalPoint.first >> arrivalPoint.second ;
        cin >> n ;
        for(int i = 0 ; i < n ; i++){
            int r ;
            pair<int, int> circle ;
            cin >> circle.first >> circle.second >> r ;
            // 같은 경우는 입력으로 주어지지 않는다.
            if(isInsideCircle(circle, startPoint, r) ^ isInsideCircle(circle, arrivalPoint, r))
                answer += 1 ;
        }
        cout << answer << "\n" ;
    }
