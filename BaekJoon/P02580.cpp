...
bool promising (int (*board)[SIZE+1], int row, int col){
    for(int i = 1 ; i <= SIZE ; i++){
        if((i != row && board[row][col] == board[i][col]) ||
        (i != col && board[row][col] == board[row][i]))
            return false ;
    }
    int rowSub = (row -1) / 3 * 3 + 1, colSub = (col -1) / 3 * 3 + 1;
    for(int r = rowSub ; r < rowSub + 3 ; r++){
        for(int c = colSub ; c < colSub + 3 ; c++){
...
void sudoku(int (*board)[SIZE+1], vector<pair<int ,int>> & blank, int k){
    if(blank.size() == k){
        ...
        for(int i = 1 ; i <= SIZE ; i++){
            board[blank[k].first][blank[k].second] = i ;
            if(promising(board, blank[k].first, blank[k].second)){
                sudoku(board, blank, k + 1) ;
                if(flag)    break ;
            }
