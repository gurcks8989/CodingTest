...
int main(){
    ...
    bool constrators[MAX + 1] ;
    // Start index is 1, end index 10000
    memset(constrators, true, sizeof(constrators));
    ...
    for(int i = 1 ; i <= MAX ; i++){
        ...
        if(i + digitSum < MAX && constrators[i+digitSum]){
            constrators[i+digitSum] = false ;
        }
    }  
    for(int i = 1 ; i < MAX ; i++){
        if(constrators[i])
